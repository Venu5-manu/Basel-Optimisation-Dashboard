...
def load_and_prepare(uploaded_file=None):
    """Load Excel (path or file-like or default), unify columns, derive Basel fields."""
    ...
    df["PD_Band"] = pd.cut(df["PD"], [0, 0.10, 0.20, 0.50, 1.0],
                           labels=["<10%", "10–20%", "20–50%", ">50%"], include_lowest=True)
    if "LTV" in df:
        df["LTV_Band"] = pd.cut(df["LTV"], [0, 0.50, 0.80, 1.00, np.inf],
                                labels=["≤50%", "50–80%", "80–100%", ">100%"])
    return df

# ----------- Data source (no uploader) -----------
# Prefer the router-provided COMBINED_XLSX if present; else default path; else small mock.
path_hint = globals().get("COMBINED_XLSX", "Basel_Combined_Datasets.xlsx")
try:
    use_path = path_hint if (isinstance(path_hint, str) and os.path.exists(path_hint)) else None
    df = load_and_prepare(use_path)
except Exception:
    # Fallback mock data ensures the page still loads
    df = pd.DataFrame({
